...
Vous êtes libres de choisir le serveur S3 auquel vous avez vous connecter.  
...
La configuration S3 est alors automatiquement injéctée via les variables d'environnement. Le confirmer via `env` : `AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, AWS_SESSION_TOKEN, AWS_DEFAULT_REGION, AWS_S3_ENDPOINT`.  
Sur ces environnements, vous avec les droits sur le bucket du nom de votre identifiant (ou, sur certaines plateformes, à un sous-path d'un bucket)
